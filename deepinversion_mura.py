...
This work is made available under the Nvidia Source Code License (1-Way Commercial). To view a copy of this license, visit https://github.com/NVlabs/$
'''
...
from resnet_mura import ResNet34, ResNet18, ResNet50
    ...
    for epoch in range(epochs):
        ...
        if competitive_scale != 0.0:
            ...
            if 1:
                ...
                eps = 0.0
                # loss_verifier_cig = 0.5 * kl_loss(F.log_softmax(outputs_verifier / T, dim=1), M) +  0.5 * kl_loss(F.log_softmax(outputs/T, dim=1), $
...
if __name__ == "__main__":
    ...
    parser.add_argument('--teacher_weights', default="teacher_resnet34_only.weights", type=str, help='path to load weights of the model')

    args = parser.parse_args()

    print("loading resnet34")

    net_teacher = load_model("resnet50")
    net_student = load_model("resnet18")
    ...
    inputs = torch.randn((args.bs, 3, 224, 224), requires_grad=True, device='cuda', dtype=data_type)
